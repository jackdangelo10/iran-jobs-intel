#!/usr/bin/env python3
"""
Test runner for JobVision scraper.
Outputs all scraped data to data/jobvision/ directory.

Usage:
    python tests/test_scrapers/run_jobvision.py
    python tests/test_scrapers/run_jobvision.py --pages 3 --jobs 10
...
def run_jobvision_test(limit_pages: int | None = None, limit_jobs: int | None = None) -> dict[str, Any]:
    """
    Run JobVision scraper and save all output.
    ...
    print("=" * 80)
    print("🇮🇷 JobVision Scraper Test Run")
    print("=" * 80)
    ...
    output_base = Path('data/jobvision')
    ...
    # Create scraper
    print("🔧 Initializing JobVision scraper...")
    scraper = JobVisionScraper(db, session_id)
    ...
    try:
        ...
        while True:
            ...
            try:
                ...
                if len(page_urls) == 0:
                    print(f"  ℹ️  No jobs found on page (end of results)")
                    break
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Test JobVision scraper and save results')
